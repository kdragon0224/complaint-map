...
- **처리**: Python 스크립트로 CSV → `public/data/highway-grid.json` 변환
  ...
  - 총 58,846개 포인트
- **성격**: 도로 **선(線)** 데이터 — 한국도로공사 ETC 시스템 기준이므로 민자 구간 일부 미포함
...
> ⚠️ **2-1과 2-2는 대소 관계가 아닌 독립 데이터**  
> 같은 구간이라도 한쪽에만 존재할 수 있음.  
> 예) 봉담-송산 구간: 노드(2-2)에는 있으나 ETC 중심선(2-1)에는 없음  
> → 핀을 찍으면 고속도로를 탐지하지 못하고 인근 국도가 대신 표시되는 한계 존재  
>
> | 상태 | 결과 |
> |------|------|
> | ETC ○ + 노드 ○ | 정상 (위치 탐색 + IC명 표시) |
> | ETC ○ + 노드 ✕ | 위치는 찾지만 IC명 모름 |
> | ETC ✕ + 노드 ○ | 고속도로 탐지 불가 → 인근 국도로 대체됨 |
> | ETC ✕ + 노드 ✕ | 결과 없음 |
